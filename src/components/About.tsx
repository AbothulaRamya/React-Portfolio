    ...
    <section id="about" className="py-24 lg:py-28">
      <div className="mx-auto max-w-6xl px-6">
        <div className="grid items-stretch gap-10 lg:grid-cols-[1.1fr_0.9fr] lg:gap-14">
          ...
          >
            <div className="flex h-full flex-col justify-between rounded-[32px] border border-slate-800/80 bg-slate-950/50 p-6">
              <p className="mb-2 text-sm text-slate-500">Location</p>
              <p className="font-semibold text-white">Parvathipuram Manyam, Andhra Pradesh</p>
            </div>
            <div className="flex h-full flex-col justify-between rounded-[32px] border border-slate-800/80 bg-slate-950/50 p-6">
              <p className="mb-2 text-sm text-slate-500">Email</p>
              <p className="break-all font-semibold text-white">{siteConfig.email}</p>
            </div>
            <div className="flex h-full flex-col justify-between rounded-[32px] border border-slate-800/80 bg-slate-950/50 p-6">
              <p className="mb-2 text-sm text-slate-500">Degree</p>
              <p className="font-semibold text-white">B.Tech — Information Technology</p>
            </div>
            <div className="flex h-full flex-col justify-between rounded-[32px] border border-slate-800/80 bg-slate-950/50 p-6">
              <p className="mb-2 text-sm text-slate-500">CGPA</p>
              <p className="font-semibold text-white">8.6 / 10</p>
            </div>
            <div className="flex h-full flex-col justify-between rounded-[32px] border border-slate-800/80 bg-slate-950/50 p-6 sm:col-span-2">
              <p className="mb-2 text-sm text-slate-500">Availability</p>
              <p className="font-semibold text-emerald-400">Open to opportunities</p>
            </div>
            <div className="flex h-full flex-col justify-between rounded-[32px] border border-slate-800/80 bg-slate-950/50 p-6 sm:col-span-2">
              <p className="mb-2 text-sm text-slate-500">GitHub</p>
              <a href={siteConfig.github} target="_blank" rel="noreferrer" className="font-semibold text-cyan-300 hover:text-white">
                @AbothulaRamya
              </a>
            </div>
          </motion.div>
